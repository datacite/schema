...
describe "validate geoLocation" do
  ...
  it 'geoLocationPoint out of range' do
    ...
    errors = xsd.validate(Nokogiri::XML(doc.to_xml)).map { |error| error.to_s }
    expect(errors.length).to eq(1)
    expect(errors.first).to eq("49:0: ERROR: Element '{https://datacite.org/schema/kernel-4}pointLatitude': [facet 'maxInclusive'] The value '110.000000' is greater than the maximum value allowed ('90').")
  end
